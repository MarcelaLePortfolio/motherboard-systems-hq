...
{
  echo "PHASE 456 — OPERATOR GUIDANCE MINIMAL PROBE (FIXED)"
  echo "Generated: $(date -u +"%Y-%m-%dT%H:%M:%SZ")"
  echo

  echo "=== OPS STREAM (FIRST 20 LINES) ==="
  curl -NsS "http://localhost:8080/events/ops" | head -20 || true
  echo

  echo "=== REFLECTIONS STREAM (FIRST 20 LINES) ==="
  curl -NsS "http://localhost:8080/events/reflections" | head -20 || true
  echo
