...
printf '%s\n' \
  ...
  'TARGET=VERIFY_DASHBOARD_PROXY_CHAT_VIA_LOCALHOST_5173'

CLIENT_CODE="$(curl -sS --max-time 5 -o /tmp/dashboard-localhost-health.html -w '%{http_code}' http://localhost:5173/ 2>/dev/null || true)"
echo "CLIENT_HTTP_STATUS=$CLIENT_CODE"
...
HTTP_CODE="$(
  curl -sS \
    --max-time 120 \
    -o /tmp/matilda-dashboard-localhost-proxy-response.json \
    -w '%{http_code}' \
    -X POST \
...
cat /tmp/matilda-dashboard-localhost-proxy-response.json 2>/dev/null || true
printf '\n'
...
if [[ "$HTTP_CODE" =~ ^2 ]] && [[ "$POST_COUNT" -gt "$PRE_COUNT" ]]; then
  printf '%s\n' \
    'DASHBOARD_PROXY_CHAT_VIA_LOCALHOST=PASS' \
  ...
  printf '%s\n' \
    'DASHBOARD_PROXY_CHAT_VIA_LOCALHOST=FAIL' \
    'ISSUE_RESOLVED=NO' \
    'NEXT_ACTION=CLASSIFY_EXACT_PROXY_FAILURE_WITHOUT_LAYERING_ANOTHER_FIX'
  exit 1
fi
