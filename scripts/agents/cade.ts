  ...
  switch (task.type) {
    ...
    case "replace": {
      ...
      try {
        ...
        result = `✅ Replaced content of "${path}"`;
      } catch (err: any) { result = "❌ Failed to replace file: " + err.message; }
      break;
    }
    ...
    case "prepend": {
      ...
      } catch (err: any) { result = "❌ Failed to prepend file: " + err.message; }
      break;
    }
    ...
    case "insert_after": {
      ...
      try {
        ...
        if (index === -1) { result = "❌ Marker not found for insert_after."; }
          ...
          fs.writeFileSync(fullPath, lines.join("\n"), "utf8");
          result = `✅ Inserted after marker in "${path}"`;
        }
    ...
    case "insert_before": {
      ...
      try {
        ...
        if (index === -1) { result = "❌ Marker not found for insert_before."; }
          ...
          fs.writeFileSync(fullPath, lines.join("\n"), "utf8");
          result = `✅ Inserted before marker in "${path}"`;
        }
