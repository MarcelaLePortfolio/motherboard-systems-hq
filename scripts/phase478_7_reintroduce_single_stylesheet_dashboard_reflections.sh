...
BACKUP="public/dashboard.html.phase478_7_pre_dashboard_reflections_restore.bak"
OUT="docs/phase478_7_reintroduce_single_stylesheet_dashboard_reflections.txt"
...
comment_line = '<!-- phase474.3 temporary neutralization: removed local stylesheet :: <link rel="stylesheet" href="css/dashboard-reflections.css" /> -->'
active_line = '<link rel="stylesheet" href="css/dashboard-reflections.css" />'

report = []
report.append("PHASE 478.7 — REINTRODUCE SINGLE STYLESHEET (dashboard-reflections.css)")
report.append("======================================================================")
report.append("")

if re.search(r'(?im)^\s*<link[^>]*href=["\']css/dashboard-reflections\.css["\'][^>]*>\s*$', text):
    report.append("ACTIVE_DASHBOARD_REFLECTIONS_CSS_ALREADY_PRESENT=1")
elif comment_line in text:
    text = text.replace(comment_line, active_line, 1)
    target_path.write_text(text)
    report.append("COMMENTED_DASHBOARD_REFLECTIONS_CSS_ACTIVATED=1")
else:
    head_close = re.search(r'(?i)</head>', text)
    if not head_close:
        report.append("ERROR: could not find </head> insertion point")
        Path("docs/phase478_7_reintroduce_single_stylesheet_dashboard_reflections.txt").write_text("\n".join(report) + "\n")
    ...
    target_path.write_text(text)
    report.append("DASHBOARD_REFLECTIONS_CSS_INSERTED_FRESH=1")
...

{
  ...
  if [ "$READY" -ne 1 ]; then
    ...
    echo "CLASSIFICATION: DASHBOARD_REFLECTIONS_CSS_REINTRODUCED"
  fi
  echo
  echo "DECISION TARGET"
  echo "- Open http://localhost:8080/dashboard.html"
  echo "- Report EXACTLY one of:"
  echo "  • DASHBOARD_REFLECTIONS_CSS_STABLE"
  echo "  • DASHBOARD_REFLECTIONS_CSS_BREAKS"
