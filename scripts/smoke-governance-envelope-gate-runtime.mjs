...
function cleanup() {
  ...
  sqlite.prepare("DELETE FROM governance_envelope_gates WHERE envelope_gate_id = ?").run("smoke-governance-envelope-gate-runtime-missing-field");

  sqlite.prepare("DELETE FROM governance_envelope_gates WHERE envelope_gate_id = ?").run("smoke-governance-envelope-gate-runtime-missing-validation");
