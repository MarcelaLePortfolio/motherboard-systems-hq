...
# find function start brace "{"
j = src.find("{", i)
if j < 0:
    raise SystemExit("patch_failed: no opening brace after appendTaskEvent signature")
...
if depth != 0:
    raise SystemExit("patch_failed: brace match failed (unbalanced braces)")

old_block = src[i:k]

new_block = """export async function appendTaskEvent(kind, task_id, payload, opts = undefined) {
  // Phase25: single authoritative task_event writer (server-side)
...
# safety: ensure old legacy insert is gone
if "insert into task_events(kind, task_id, payload) values" in patched:
    raise SystemExit("patch_failed: legacy insert still present after replacement")
