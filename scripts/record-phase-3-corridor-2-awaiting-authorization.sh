...
echo "=== PHASE 3 / CORRIDOR 2 — AWAITING AUTHORIZATION ==="

test "$(git branch --show-current)" = "feature/support-source-references-runtime"
git merge-base --is-ancestor e3d4ab76 HEAD

unexpected="$(
  git status --porcelain |
  grep -vE '^\?\? scripts/record-phase-3-corridor-2-awaiting-authorization\.sh$|^ M scripts/record-phase-3-corridor-2-awaiting-authorization\.sh$' ||
  true
)"
test -z "$unexpected"
...
DR_NOW=NO
NO_FURTHER_GATE_CHECKPOINTS_REQUIRED=YES
MAP
