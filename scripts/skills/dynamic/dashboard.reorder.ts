// Auto-generated dynamic skill for "dashboard.reorder"
...
export default async function run(params: Params, ctx: { dryRun?: boolean }) {
  if (ctx?.dryRun) {
    return `(dry-run) would perform: dashboard.reorder with ${JSON.stringify(params)}`;
  }
  // TODO: replace with real logic. Keep file-system/network changes minimal here.
  return `✅ dashboard.reorder completed with ${JSON.stringify(params)}`;
}
