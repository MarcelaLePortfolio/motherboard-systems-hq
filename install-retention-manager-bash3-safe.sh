...
REPORT="retention-manager-bash3-safe-install-$(date +%Y%m%d_%H%M%S).md"

SYSTEM_DIR="$HOME/motherboard-backup-system"

MANAGER="$SYSTEM_DIR/snapshot-manager-prod.sh"

BACKUP_COPY="$SYSTEM_DIR/snapshot-manager-prod.sh.pre-bash3-safe-$(date +%Y%m%d_%H%M%S)"
...
for BASE in "/Volumes/Rio Drive/backups" "/Volumes/Rio Drive/Motherboard_External_Backup/snapshots" "/Users/marcela-dev/Projects/motherboard-systems-hq-clean/backups"; do
  ...
  FILE_LIST="$SYSTEM_DIR/.retention-files-$(echo "$BASE" | shasum -a 256 | awk '{print $1}').txt"

  find "$BASE" -maxdepth 1 -type f \( -name "*.tar.gz" -o -name "*.bundle" \) -print 2>/dev/null | while read -r f; do
...

{
  ...
  "mode": "safe_autonomous_delete_only_retention_bash3",
...

{
  ...
  echo "# Retention Manager Bash 3 Safe Install"
...
git add "$REPORT" install-retention-manager-bash3-safe.sh

git commit -m "Install bash3 safe retention manager"
