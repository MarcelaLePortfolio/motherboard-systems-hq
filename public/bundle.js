(() => {
  ...
  (() => {
    ...
    const TASK_POLL_MS = 15e3;
    const RUNS_POLL_MS = 15e3;
    ...
    function render() {
      ...
      if (!ui) return;
      if (ui.errorEl) {
        ui.errorEl.innerHTML = state.lastError ? `<div style="color:red">${esc(state.lastError)}</div>` : "";
      }
      if (ui.listEl) {
        ui.listEl.innerHTML = `
        <div>
          ${state.loading && !state.tasks.length ? `<div style="opacity:.7">Loading\u2026</div>` : ""}
          ${state.tasks.length ? state.tasks.map(
          (t) => `
              <div style="display:flex;justify-content:space-between;gap:8px">
                <span>${esc(t.title)}</span>
                ${["complete", "completed", "done"].includes(String(t.status || "").toLowerCase()) ? `<span style="opacity:.5;font-size:12px">Completed</span>` : `<button data-id="${esc(t.id)}">Complete</button>`}
              </div>
            `
        ).join("") : !state.loading ? `<div style="opacity:.7">No tasks found.</div>` : ""}
        </div>
      `;
      ...
      function readFilters() {
        ...
        if (!since_ts && lastMin) {
          ...
          if (Number.isFinite(n) && n > 0) {
            since_ts = String(Date.now() - Math.floor(n * 6e4));
