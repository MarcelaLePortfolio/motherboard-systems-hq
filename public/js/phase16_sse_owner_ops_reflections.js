...
// Create once, reuse forever.
    if (!window.__opsES) {
      try {
        window.__opsES = window.connectSSEWithBackoff({url:"/events/ops",isHealthyEvent:(n)=>n==="heartbeat"||n==="hello"||n==="ops.state"});
      } catch (e) {
...
if (!(window.__refES && typeof window.__refES.addEventListener === "function")) {
      try {
        ...
        window.__refES = window.connectSSEWithBackoff({url:"/events/reflections",isHealthyEvent:(n)=>n==="heartbeat"||n==="hello"||n==="reflections.state"});
