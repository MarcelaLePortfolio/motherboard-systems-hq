...
echo "===== PHASE 719: VALIDATE SHARED ARTIFACT VOLUME WITH NEW TASK ====="

mkdir -p checkpoints

BRANCH="$(git branch --show-current)"

OUT="checkpoints/PHASE719_SHARED_ARTIFACT_VOLUME_NEW_TASK_VERIFY.txt"
...
CREATE_RESPONSE="$(curl -s --max-time 10 \
  ...
  -d "{\"title\":\"$TEST_TITLE\",\"agent\":\"cade\",\"source\":\"phase719-preview-validation\"}" || true)"

echo "$CREATE_RESPONSE" > checkpoints/PHASE719_SHARED_ARTIFACT_VOLUME_NEW_TASK_CREATE_RESPONSE.json
...
for i in $(seq 1 30); do
  ...
  STATUS="$(curl -s --max-time 10 http://localhost:3000/api/tasks | python3 -c 'import json,sys,os
...

{
  ...
  echo "PHASE 719 SHARED ARTIFACT VOLUME NEW TASK VERIFY"
...
git add PHASE719_VALIDATE_SHARED_ARTIFACT_VOLUME_WITH_NEW_TASK.sh

git add checkpoints/PHASE719_SHARED_ARTIFACT_VOLUME_NEW_TASK_CREATE_RESPONSE.json
