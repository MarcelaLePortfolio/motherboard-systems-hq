...
echo "===== PHASE 719: ADD HTML ARTIFACT AND RENDER DIRECTLY V2 ====="
...
OUT="checkpoints/PHASE719_HTML_ARTIFACT_DIRECT_RENDER_V2_VERIFY.txt"

cp "$WORKER" checkpoints/PHASE719_WORKER_PRE_HTML_ARTIFACT_DIRECT_RENDER_V2.mjs

cp "$ROUTE" checkpoints/PHASE719_ROUTE_PRE_HTML_ARTIFACT_DIRECT_RENDER_V2.mjs

cp "$FRONTEND" checkpoints/PHASE719_PHASE530_PRE_HTML_ARTIFACT_DIRECT_RENDER_V2.js
...
if old2 not in text:
    ...
    raise SystemExit("worker emit artifact payload block not found")
...
if old2 not in text:
    ...
    raise SystemExit("route response artifact content block not found")
...
cp "$WORKER" checkpoints/PHASE719_WORKER_POST_HTML_ARTIFACT_DIRECT_RENDER_V2.mjs

cp "$ROUTE" checkpoints/PHASE719_ROUTE_POST_HTML_ARTIFACT_DIRECT_RENDER_V2.mjs

cp "$FRONTEND" checkpoints/PHASE719_PHASE530_POST_HTML_ARTIFACT_DIRECT_RENDER_V2.js

docker compose up -d --build worker dashboard

sleep 6

cat > /tmp/phase719_html_artifact_v2_body.json << 'JSON'

{"title":"Phase 719 HTML artifact validation: create a direct embedded preview artifact","agent":"cade","source":"phase719-html-artifact-validation-v2"}

JSON

CREATE_RESPONSE="$(curl -sS --max-time 15 -X POST 'http://localhost:3000/api/tasks/create' -H 'Content-Type: application/json' --data-binary @/tmp/phase719_html_artifact_v2_body.json || true)"

printf '%s\n' "$CREATE_RESPONSE" > checkpoints/PHASE719_HTML_ARTIFACT_V2_CREATE_RESPONSE.json
...
git add PHASE719_ADD_HTML_ARTIFACT_AND_RENDER_DIRECTLY_V2.sh

git add checkpoints/PHASE719_WORKER_PRE_HTML_ARTIFACT_DIRECT_RENDER_V2.mjs

git add checkpoints/PHASE719_ROUTE_PRE_HTML_ARTIFACT_DIRECT_RENDER_V2.mjs

git add checkpoints/PHASE719_PHASE530_PRE_HTML_ARTIFACT_DIRECT_RENDER_V2.js

git add checkpoints/PHASE719_WORKER_POST_HTML_ARTIFACT_DIRECT_RENDER_V2.mjs

git add checkpoints/PHASE719_ROUTE_POST_HTML_ARTIFACT_DIRECT_RENDER_V2.mjs

git add checkpoints/PHASE719_PHASE530_POST_HTML_ARTIFACT_DIRECT_RENDER_V2.js

git add checkpoints/PHASE719_HTML_ARTIFACT_V2_CREATE_RESPONSE.json
...
echo "===== HTML ARTIFACT DIRECT RENDER V2 COMPLETE ====="
