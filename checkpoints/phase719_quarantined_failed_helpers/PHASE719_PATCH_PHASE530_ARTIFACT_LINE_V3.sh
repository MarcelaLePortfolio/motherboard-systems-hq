...
echo "===== PHASE 719: PATCH PHASE530 ARTIFACT LINE V3 ====="

mkdir -p checkpoints

TARGET="public/js/phase530_visible_panels_bridge.js"

cp "$TARGET" checkpoints/PHASE719_PHASE530_PRE_ARTIFACT_LINE_V3.js
...
needle_render = '            <div style="margin-top:10px;border:1px solid rgba(71,85,105,.55);border-radius:12px;padding:9px;background:rgba(2,6,23,.24);">\n'

insert_render = '            ${artifactRaw ? `<div style="margin-top:8px;color:#86efac;font-size:11px;line-height:1.5;overflow-wrap:anywhere;border:1px solid rgba(134,239,172,.28);border-radius:10px;padding:7px;background:rgba(20,83,45,.14);">Artifact: ${artifactName}${artifactType ? ` · ${artifactType}` : ""}${artifactSize ? ` · ${artifactSize}` : ""}</div>` : ""}\n\n'
...
cp "$TARGET" checkpoints/PHASE719_PHASE530_POST_ARTIFACT_LINE_V3.js
...
} | tee checkpoints/PHASE719_PHASE530_ARTIFACT_LINE_V3_VERIFY.txt
...
git add "$TARGET" \
  ...
  checkpoints/PHASE719_PHASE530_POST_ARTIFACT_LINE_V3.js \

  checkpoints/PHASE719_PHASE530_ARTIFACT_LINE_V3_VERIFY.txt

git commit -m "Phase 719: surface artifact line before operator actions"

git push origin "$(git branch --show-current)"

echo "===== PHASE530 ARTIFACT LINE V3 PATCH COMPLETE ====="
