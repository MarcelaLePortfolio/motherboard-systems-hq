...
echo "===== PHASE 719: PATCH PHASE530 ARTIFACT LINE V4 ====="

mkdir -p checkpoints

TARGET="public/js/phase530_visible_panels_bridge.js"

cp "$TARGET" checkpoints/PHASE719_PHASE530_PRE_ARTIFACT_LINE_V4.js
...
needle_render = '              <div style="color:#cbd5e1;font-size:11px;font-weight:700;margin-bottom:7px;letter-spacing:.02em;">Operator actions</div>\n'

insert_render = '              ${artifactRaw ? `<div style="margin-bottom:8px;color:#86efac;font-size:11px;line-height:1.5;overflow-wrap:anywhere;border:1px solid rgba(134,239,172,.28);border-radius:10px;padding:7px;background:rgba(20,83,45,.14);">Artifact: ${artifactName}${artifactType ? ` · ${artifactType}` : ""}${artifactSize ? ` · ${artifactSize}` : ""}</div>` : ""}\n'

if "Artifact: ${artifactName}" not in text:

    if needle_render not in text:

        raise SystemExit("Operator actions insertion point not found.")

    text = text.replace(needle_render, insert_render + needle_render, 1)
...
cp "$TARGET" checkpoints/PHASE719_PHASE530_POST_ARTIFACT_LINE_V4.js
...
} | tee checkpoints/PHASE719_PHASE530_ARTIFACT_LINE_V4_VERIFY.txt
...
git add "$TARGET" \
  ...
  checkpoints/PHASE719_PHASE530_POST_ARTIFACT_LINE_V4.js \

  checkpoints/PHASE719_PHASE530_ARTIFACT_LINE_V4_VERIFY.txt

git commit -m "Phase 719: surface artifact metadata inside recent task cards"

git push origin "$(git branch --show-current)"

echo "===== PHASE530 ARTIFACT LINE V4 PATCH COMPLETE ====="
