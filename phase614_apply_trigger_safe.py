...
p = Path("server.mjs")
if not p.exists():
    raise SystemExit("server.mjs not found")

s = p.read_text()

marker = "// PHASE614 SYSTEM TRIGGER (SAFE, REVERSIBLE)"
if marker in s:
    raise SystemExit("Phase614 trigger already present")
...
# Append safely to end of file
p.write_text(s.rstrip() + "\n" + block)

print("Phase614 trigger appended safely.")
