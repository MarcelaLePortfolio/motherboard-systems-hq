...
/* Phase 576 SAFE OVERRIDE — Retry Differently (non-destructive) */
(function () {
  document.addEventListener("click", async (e) => {
    const el = e.target.closest('[data-action="retry-different"]');
    if (!el) return;
    ...
    // debug hook (safe)
    console.log("[retry-different][override] click detected");
    ...
    // IMPORTANT: do not interfere with existing handlers
    // just observe + forward safely
    ...
    try {
      ...
      const data = await res.json();
      console.log("[retry-different][override] queued:", data);
    } catch (err) {
      console.error("[retry-different][override] failed:", err);
    }
  });
})();
  ...
  function getRoot() {
    return document.body; // safest stable root in your current architecture
  }

  getRoot().addEventListener("click", async (e) => {
    const el = e.target.closest('[data-action="retry-different"]');
    if (!el) return;

    console.log("[retry-different] click detected (delegated root)");
...
/* Phase 576 FIX — derive task from clicked row instead of global state */
document.body.addEventListener("click", async (e) => {
  ...
  // 🔑 CRITICAL FIX: derive from DOM instead of window.selectedItem
  const row = el.closest("[data-task-id]");
  if (!row) {
    console.warn("[retry-different] no task row found");
    return;
  }
  ...
  const body = {
    ...
    meta: {
      retry_mode: "strategy_shift",
      strategy_applied: "prompt_augmentation",
      retry_of_task_id: task.taskId,
      retry_of_event_id: task.id,
...
/* Phase 576 SAFE FIX — rebind after inspector render */
function bindRetryDifferentlyAfterRender() {
  setTimeout(() => {
    document.querySelectorAll('[data-action="retry-different"]').forEach(el => {
...
// run initially
bindRetryDifferentlyAfterRender();

// observe DOM changes (critical for SSE UI)
...
observer.observe(document.body, {
  ...
  document.querySelectorAll('[data-action="retry-different"]').forEach(el => {
    ...
    // ensure clickable state
    el.style.pointerEvents = "auto";
    el.style.cursor = "pointer";

    // prevent duplicate binding
    if (el.__retryBound) return;
    el.__retryBound = true;

    el.addEventListener("click", async (e) => {
      e.stopPropagation();

      console.log("[retry-different] click (render-bound)");
