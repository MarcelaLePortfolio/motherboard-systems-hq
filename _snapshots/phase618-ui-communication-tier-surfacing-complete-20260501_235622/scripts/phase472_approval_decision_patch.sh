...
APPROVAL_FAILURE_FILE="docs/proofs/approval_failure_${PLAN_ID}.json"
EXECUTION_FILE="docs/proofs/execution_${PLAN_ID}.json"
FAILURE_FILE="docs/proofs/failure_${INTAKE_ID}.json"

rm -f "${FAILURE_FILE}" "${GOVERNANCE_FAILURE_FILE}" "${APPROVAL_FAILURE_FILE}" "${APPROVAL_FILE}" "${EXECUTION_FILE}"
...
if [[ "${NORMALIZED_INPUT}" == echo\ reject\ * ]]; then
  ...
  cat > "${APPROVAL_FAILURE_FILE}" <<JSON
{
  ...
  "stage": "approval",
  "decision": "REJECTED",
  "reason": "OPERATOR_REJECTED_PATTERN"
}
JSON
