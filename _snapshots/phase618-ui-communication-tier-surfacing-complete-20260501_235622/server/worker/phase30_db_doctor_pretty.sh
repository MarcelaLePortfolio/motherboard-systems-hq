...
select
  ...
  (select data_type from information_schema.columns where table_name='task_events' and column_name='payload')::text as events_payload_type,
...
echo
if [ "${ev_payload_jsonb:-0}" -ge 1 ]; then
