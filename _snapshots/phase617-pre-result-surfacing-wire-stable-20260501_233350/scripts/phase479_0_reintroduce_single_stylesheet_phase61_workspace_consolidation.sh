...
BACKUP="public/dashboard.html.phase479_0_pre_phase61_workspace_consolidation_restore.bak"
OUT="docs/phase479_0_reintroduce_single_stylesheet_phase61_workspace_consolidation.txt"
...
comment_line = '<!-- phase474.3 temporary neutralization: removed local stylesheet :: <link rel="stylesheet" href="css/phase61_workspace_consolidation.css" /> -->'
active_line = '<link rel="stylesheet" href="css/phase61_workspace_consolidation.css" />'

report = []
report.append("PHASE 479.0 — REINTRODUCE SINGLE STYLESHEET (phase61_workspace_consolidation.css)")
report.append("=================================================================================")
report.append("")

if re.search(r'(?im)^\s*<link[^>]*href=["\']css/phase61_workspace_consolidation\.css["\'][^>]*>\s*$', text):
    report.append("ACTIVE_PHASE61_WORKSPACE_CONSOLIDATION_CSS_ALREADY_PRESENT=1")
elif comment_line in text:
    text = text.replace(comment_line, active_line, 1)
    target_path.write_text(text)
    report.append("COMMENTED_PHASE61_WORKSPACE_CONSOLIDATION_CSS_ACTIVATED=1")
else:
    head_close = re.search(r'(?i)</head>', text)
    if not head_close:
        report.append("ERROR: could not find </head> insertion point")
        Path("docs/phase479_0_reintroduce_single_stylesheet_phase61_workspace_consolidation.txt").write_text("\n".join(report) + "\n")
    ...
    target_path.write_text(text)
    report.append("PHASE61_WORKSPACE_CONSOLIDATION_CSS_INSERTED_FRESH=1")
...

{
  ...
  if [ "$READY" -ne 1 ]; then
    ...
    echo "CLASSIFICATION: PHASE61_WORKSPACE_CONSOLIDATION_CSS_REINTRODUCED"
  fi
  echo
  echo "DECISION TARGET"
  echo "- Open http://localhost:8080/dashboard.html"
  echo "- Report EXACTLY one of:"
  echo "  • PHASE61_WORKSPACE_CSS_STABLE"
  echo "  • PHASE61_WORKSPACE_CSS_BREAKS"
