...
OUT="docs/phase456_run_view_repair_retry_correct_paths.txt"

{
  echo "PHASE 456 — RUN_VIEW REPAIR RETRY (CORRECT PATHS)"
  ...
  echo "=== VERIFY CORRECT RUN_VIEW FILES IN REPO ==="
  ls -l drizzle_pg/0007_phase36_1_run_view.sql
  ls -l drizzle_pg/0007_phase36_2_run_view_contract.sql
  echo

  echo "=== VERIFY CORRECT RUN_VIEW FILES IN DASHBOARD CONTAINER ==="
  docker compose exec -T dashboard sh -lc 'ls -l /app/drizzle_pg/0007_phase36_1_run_view.sql /app/drizzle_pg/0007_phase36_2_run_view_contract.sql'
  echo
  ...
  if [ "${RUN_VIEW_EXISTS}" = "0" ]; then
    ...
    docker compose exec -T postgres psql -U "${PGUSER:-postgres}" -d "${PGDATABASE:-postgres}" -v ON_ERROR_STOP=1 < drizzle_pg/0007_phase36_1_run_view.sql
    docker compose exec -T postgres psql -U "${PGUSER:-postgres}" -d "${PGDATABASE:-postgres}" -v ON_ERROR_STOP=1 < drizzle_pg/0007_phase36_2_run_view_contract.sql
  else
