...
echo "=== Phase 22: wire /events/task-events into dashboard UI ==="
mkdir -p public/js

cat > public/js/task-events-sse-client.js <<'JS'
(() => {
...
if [ -f "$DASH_HTML" ]; then
  if rg -n 'task-events-sse-client\.js' "$DASH_HTML" >/dev/null 2>&1; then
    echo "dashboard.html already loads task-events-sse-client.js"
  else
    echo "Injecting <script> for task-events-sse-client.js into public/dashboard.html"
    perl -0777 -i -pe 's@</body>@  <script src="/js/task-events-sse-client.js"></script>\n</body>@s' "$DASH_HTML"
  fi
else
  echo "WARN: public/dashboard.html not found; skipping HTML injection."
fi
CANDIDATES=(
  "public/js/dashboard-bundle-entry.js"
  "public/js/dashboard-bundle-entry.mjs"
  "public/js/dashboard-bundle-entry.ts"
  "public/js/dashboard-bundle-entry.tsx"
  "public/js/dashboard-bundle-entry.jsx"
  "public/js/dashboard-bundle-entry.mts"
  "public/js/dashboard-bundle-entry.cts"
)

for f in "${CANDIDATES[@]}"; do
  if [ -f "$f" ]; then
    if rg -n 'task-events-sse-client' "$f" >/dev/null 2>&1; then
      echo "bundle entry already references task-events-sse-client: $f"
    else
      echo "Appending import for task-events-sse-client into: $f"
      printf '\n// Phase 22: task-events live UI\nimport "./task-events-sse-client.js";\n' >> "$f"
    fi
  fi
done

echo "OK: Phase 22 wiring complete."
echo "Next: rebuild dashboard bundle (if applicable) and verify live updates in browser."
