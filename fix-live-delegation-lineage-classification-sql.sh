...
cat > classify-live-delegation-migration-lineage-compatibility.sh << 'INNER'
#!/usr/bin/env bash
set -euo pipefail
cd "$(git rev-parse --show-toplevel)"
...
chmod +x classify-live-delegation-migration-lineage-compatibility.sh
git add classify-live-delegation-migration-lineage-compatibility.sh
git commit -m "Fix Delegation lineage classification SQL"
git push
./classify-live-delegation-migration-lineage-compatibility.sh
