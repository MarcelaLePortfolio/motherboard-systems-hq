...
cat > inspect-current-delegation-runtime-fragments.sh << 'INNER'
#!/usr/bin/env bash
set -euo pipefail
cd "$(git rev-parse --show-toplevel)"
...
chmod +x inspect-current-delegation-runtime-fragments.sh
git add inspect-current-delegation-runtime-fragments.sh
git commit -m "Inspect current Delegation runtime fragments"
git push
./inspect-current-delegation-runtime-fragments.sh
