...
echo "===== PHASE 719: WIRE PREVIEW MODAL TO ARTIFACT CONTENT V2 ====="
...
OUT="checkpoints/PHASE719_PREVIEW_MODAL_CONTENT_FETCH_V2_VERIFY.txt"

cp "$TARGET" checkpoints/PHASE719_PHASE530_PRE_PREVIEW_MODAL_CONTENT_FETCH_V2.js
...
start = text.find("  function phase719OpenPreviewModal(button) {")

if start == -1:

    raise SystemExit("phase719OpenPreviewModal start not found")

end = text.find("\n  document.addEventListener(\"click\", function (event) {", start)

if end == -1:

    raise SystemExit("click listener marker not found")

new_func = '''  async function phase719OpenPreviewModal(button) {
...
cp "$TARGET" checkpoints/PHASE719_PHASE530_POST_PREVIEW_MODAL_CONTENT_FETCH_V2.js
...

{
  ...
  curl -s --max-time 10 'http://localhost:3000/js/phase530_visible_panels_bridge.js' | grep -nE 'artifact-preview|Loading rendered artifact preview|Rendered artifact content is not available|Preview fetch failed|async function phase719OpenPreviewModal' | head -n 60 || true
...
git add PHASE719_WIRE_PREVIEW_MODAL_TO_ARTIFACT_CONTENT_V2.sh

git add checkpoints/PHASE719_PHASE530_PRE_PREVIEW_MODAL_CONTENT_FETCH_V2.js

git add checkpoints/PHASE719_PHASE530_POST_PREVIEW_MODAL_CONTENT_FETCH_V2.js
...
echo "===== PREVIEW MODAL CONTENT FETCH V2 COMPLETE ====="
