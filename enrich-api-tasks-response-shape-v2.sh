...
REPORT="TASK_RESPONSE_ENRICHMENT_V2.txt"
...
if count != 1:
    ...
    raise SystemExit(f"expected exactly one /api/tasks query block replacement, got {count}")
...

{
  ...
  echo "===== TASK RESPONSE ENRICHMENT V2 ====="
...
echo "===== VERIFY ENRICHED /api/tasks =====" | tee -a "$REPORT"
...
git add server/routes/api-tasks-postgres.mjs enrich-api-tasks-response-shape-v2.sh "$REPORT"
