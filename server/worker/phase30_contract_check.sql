...
begin
  ...
  if not exists (select 1 from information_schema.columns where table_name='task_events' and column_name='payload') then
    raise exception 'missing task_events.payload';
  end if;
end $$;
