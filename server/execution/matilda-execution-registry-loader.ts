...
export type CadeExecutionState =
  | "DISABLED"
  | "ARMED"
  | "READY"
  | "EXECUTABLE";
...
export type CadeExecutionRegistry = {
  ...
  execution_state_model: {
    states: CadeExecutionState[];
    final_state_is_derived_only: boolean;
    transitions: Record<CadeExecutionState, CadeExecutionState[]>;
  };
  invariants: {
...
export function loadCadeExecutionRegistry(): CadeExecutionRegistry {
  ...
  return JSON.parse(raw) as CadeExecutionRegistry;
}
