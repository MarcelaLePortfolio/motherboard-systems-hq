...
Add a deterministic **scope gate** so Phase 42 PRs cannot merge while the scope doc still contains `TBD`.
...
- `./scripts/phase42_scope_gate.sh` fails when `PHASE42_SCOPE.md` contains `TBD`
- the same script passes when no `TBD` remains
