...
echo "===== PHASE 716 PATCH RECENT TASKS RENDERER CONTAINMENT ====="
...
start = src.find("  function taskRows(tasks) {")

if start == -1:

    raise SystemExit("Could not find taskRows renderer")

end = src.find("\n  function renderRecentTasks", start)

if end == -1:

    raise SystemExit("Could not find renderRecentTasks boundary")

replacement = r'''  function taskRows(tasks) {
...
echo "===== PHASE 716 PATCH RECENT TASKS RENDERER CONTAINMENT COMPLETE ====="
