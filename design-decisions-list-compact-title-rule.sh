...
echo "=== DESIGN DECISIONS LIST COMPACT TITLE RULE ==="
echo "EXPECTED_HEAD_PREFIX=f8a92a98d"
...
if [[ "${CURRENT_HEAD}" != f8a92a98d* ]]; then
  echo "UNEXPECTED_HEAD=${CURRENT_HEAD}"
  exit 1
fi

cat > DECISIONS_LIST_COMPACT_TITLE_RULE_DESIGN.md << 'DESIGN'
...
echo "IMPLEMENTATION_AUTHORIZED=NO"
echo "NEXT_ACTION=REVIEW_AND_AUTHORIZE_PRESENTATION_ONLY_COMPACT_TITLE_IMPLEMENTATION"

git diff --check

git add \
  DECISIONS_LIST_COMPACT_TITLE_RULE_DESIGN.md \
  design-decisions-list-compact-title-rule.sh

git commit -m "Design decisions list compact title rule"
git push
