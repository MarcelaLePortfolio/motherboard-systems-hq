...
function executeDelegation(task) {
    ...
    // Mark as completed
    sqlite.prepare("UPDATE task_events SET status = 'completed' WHERE id = ?").run(task.id);
...
function checkPendingDelegations() {
    const pending = sqlite.prepare("SELECT id, description FROM task_events WHERE event_type = 'delegation' AND status = 'pending'").all();
    for (const task of pending)
        executeDelegation(task);
}
