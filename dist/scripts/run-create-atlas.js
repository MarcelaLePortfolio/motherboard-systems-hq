...
function logReflection(message) {
    const timestamp = new Date().toISOString();
    const entry = { timestamp, message };
    fs.mkdirSync(reflectionsDir, { recursive: true });
    fs.appendFileSync(path.join(reflectionsDir, `${agentName}.log`), JSON.stringify(entry) + "\n");
    console.log("🪞", message);
}
...
`;
    ...
    try {
        execSync(`pm2 start scripts/_local/agent-runtime/launch-${agentName}.ts --name ${agentName} --interpreter $(which tsx)`, { stdio: "inherit" });
        execSync("pm2 save", { stdio: "inherit" });
        logReflection("Effie registered Atlas with PM2 and saved state.");
    }
    catch (err) {
        logReflection("⚠️ PM2 registration failed: " + err.message);
        process.exit(1);
    }
