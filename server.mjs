  ...
  try {
    ...
    try {
      ...
      if (db) {
        ...
        if (r.rows && r.rows.length > 0) {
          const row = r.rows[0];
          agentAssignment = row.claimed_by || "unassigned";
    ...
    try {
      const ollamaRes = await fetch("http://host.docker.internal:11434/api/generate", {
        ...
        body: JSON.stringify({
          model: "gemma3:4b",
          prompt: `You are Matilda, the Motherboard Systems operator assistant. Reply conversationally, briefly, and safely. You may use only this read-only context. Do not claim to execute actions. Operator message: "${message}". Context: ${runSummary || "No recent run context."} Waiting on: ${waitingOn || "unknown"}. Task claimed by: ${agentAssignment || "unassigned"}. Run view agent: ${runAgent || "unknown"}. Note: If task is unassigned, no agent is responsible for execution. Do not attribute waiting or delay to any agent in that case.`,
          stream: false,
        }),
      });
